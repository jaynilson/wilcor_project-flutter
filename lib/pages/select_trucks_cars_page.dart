...
class SelectTrucksCarsPage extends StatefulWidget {
  Widget afterPage;
  SelectTrucksCarsPage(this.afterPage, {super.key});

  @override
  State<SelectTrucksCarsPage> createState() => _SelectTrucksCarsPageState();
}

class _SelectTrucksCarsPageState extends State<SelectTrucksCarsPage> {
  ...
  Future<Null> loadMore() {
    ...
    return WebService(context)
        .getFleets(limit, fleets.length, "trucks_cars", place, context,
            provider.user.token ?? "",
            search: cSearch.text)
        .then((value) {
      if (value.length > 0) {
  ...
  Future<Null> load() {
    ...
    return WebService(context)
        .getFleets(
            limit, 0, "trucks_cars", place, context, provider.user.token ?? "",
            search: cSearch.text)
        .then((value) {
      if (mounted)
        setState(() {
