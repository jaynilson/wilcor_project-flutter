...
import 'package:wilcoerp/pages/auto_incident_step7_page.dart';
import 'package:wilcoerp/pages/employee_incident_injury_report_step2_page.dart';
import 'package:wilcoerp/pages/utility_damage_step4_page.dart';
import 'package:wilcoerp/providers/app.dart';
import 'package:wilcoerp/services/web_service.dart';

class AutoIncidentStep6Page extends StatefulWidget {
  AutoIncidentStep6Page({super.key});

  @override
  State<AutoIncidentStep6Page> createState() => _AutoIncidentStep6PageState();
}

class _AutoIncidentStep6PageState extends State<AutoIncidentStep6Page> {
  ...
  next() async {
    ...
    simpleLoading(context, (BuildContext loadingContext) {
      ...
      WebService(context)
          .accidentReport("auto incident", provider.user.id.toString(),
              provider.user.token ?? "")
          .then((data) async {
        id_report_auto_incident = data["id"].toString();
        for (var i = 0; i < report_auto_incident.length; i++) {
          dynamic item = report_auto_incident[i];
        ...
        report_auto_incident = [];
        Navigator.pop(loadingContext);

        Navigator.push(
            context,
            PageTransition(
                child: AutoIncidentStep7Page(),
