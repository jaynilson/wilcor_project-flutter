...
class SelectTrailersPage extends StatefulWidget {
  Widget afterPage;
  SelectTrailersPage(this.afterPage, {super.key});

  @override
  State<SelectTrailersPage> createState() => _SelectTrailersPageState();
}

class _SelectTrailersPageState extends State<SelectTrailersPage> {
  ...
  Future<Null> loadMore() {
    ...
    return WebService(context)
        .getFleets(limit, fleets.length, "trailers", place, context,
            provider.user.token ?? "",
            search: cSearch.text)
        .then((value) {
      if (value.length > 0) {
  ...
  Future<Null> load() {
    ...
    return WebService(context)
        .getFleets(
            limit, 0, "trailers", place, context, provider.user.token ?? "",
            search: cSearch.text)
        .then((value) {
      if (mounted)
        setState(() {
